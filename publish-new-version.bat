...
echo.
echo [1/5] A limpar compilacoes antigas...
if exist dist (
    rd /s /q dist
)
...
echo.
echo [3/5] A verificar o Instalador gerado...
...
powershell -Command "$p='FieldVision-website/index.html'; if (Test-Path $p) { $c = Get-Content $p -Raw; $c = $c -replace 'releases/download/v[\d\.]+/FieldVision[-_a-zA-Z0-9\.]+\.(zip|exe)', 'releases/download/%VERSION%/FieldVision-Setup-%VERSION_NUM%.exe'; Set-Content $p -Value $c; Write-Host 'Link atualizado com sucesso!' } else { Write-Host 'Aviso: index.html do site nao encontrado.' }"

echo.
echo [5/5] A enviar o Instalador (.exe) para as Releases do GitHub e a publicar o site...
call gh release create %VERSION% "dist/FieldVision-Setup-%VERSION_NUM%.exe" --repo joaopakina14/FieldVision --title "FieldVision %VERSION% (Windows)" --notes "Lancamento oficial da versao %VERSION%."
if %ERRORLEVEL% NEQ 0 (
    echo.
...
echo.
echo ====================================================
echo   VERSAO %VERSION% PUBLICADA COM SUCESSO!
echo ====================================================
echo.
pause
